...
def extract_two_hand_features(result):
    ...
    for hand_idx in range(2):
        if hand_idx < len(result.hand_landmarks):
            ...
            # compute list of (x,y) pairs for this hand
            if NORMALIZE_LANDMARKS:
                wrist_x, wrist_y = lm[0].x, lm[0].y
                hand_pairs = [(p.x - wrist_x, p.y - wrist_y) for p in lm]
            else:
                hand_pairs = [(p.x, p.y) for p in lm]
            # flatten and extend into features
            features.extend([coord for pair in hand_pairs for coord in pair])
        else:
