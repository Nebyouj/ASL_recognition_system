...
class ASLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if x.shape != (SEQUENCE_LENGTH, INPUT_SIZE):
            raise ValueError(
                f"Sample {path} has shape {x.shape}, expected "
                f"({SEQUENCE_LENGTH}, {INPUT_SIZE})"
            )
...
class ASLBiLSTM(nn.Module):
    def __init__(self, input_size=INPUT_SIZE, hidden_size=128, num_classes=4):
        super().__init__()
        self.lstm = nn.LSTM(
...
model = ASLBiLSTM(input_size=INPUT_SIZE, num_classes=len(dataset.labels))
